	...
	if(typeof aimgcount == 'object' && $('imagelistthumb_' + pid)) {
		for(pid in aimgcount) {
			...
			for(i = 0;i < aimgcount[pid].length;i++) {
				...
				if(fid) {
					imagelist += '<div id="pattimg_' + aimgcount[pid][i] + '_menu" class="tip tip_4" style="display: none;"><div class="tip_horn"></div><div class="tip_c"><a href="forum.php?mod=ajax&action=setthreadcover&aid=' + aimgcount[pid][i] + '&fid=' + fid + '" class="xi2" onclick="showWindow(\'setcover' + aimgcount[pid][i] + '\', this.href)">��Ϊ����</a></div></div>';
				}
				imagelist += '<div class="pattimg">' +
					'<a id="pattimg_' + aimgcount[pid][i] + '" class="pattimg_zoom" href="javascript:;"' + s + ' onclick="zoom($(\'aimg_' + aimgcount[pid][i] + '\'), attachimggetsrc(\'aimg_' + aimgcount[pid][i] + '\'), 0, 0, ' + (parseInt(showexif) ? 1 : 0) + ')" title="����Ŵ�">����Ŵ�</a>' +
...
function replyNotice() {
	...
	if(status == 1) {
		...
		replynotice.innerHTML = '���ջظ�֪ͨ';
		replynotice.setAttribute("status", 0);
	} else {
		replynotice.href = newurl + 'ignore';
		replynotice.innerHTML = 'ȡ��ظ�֪ͨ';
		...
		if(connect_uin) {
			setTimeout(function () {
				if(!connect_share_loaded) {
					showDialog('�����������ʧ�ܣ����Ժ����ԡ�', 'notice');
					$('append_parent').removeChild($('connect_load_js'));
				}
			}, 5000);
			connect_load(connect_share_url);
		} else {
			showDialog($('connect_share_unbind').innerHTML, 'info', '���Ȱ�QQ�˺�');
		}
		return false;
	}
}
...
function connect_get_thread() {
	...
	if ($('postmessage_' + connect_thread_info.post_id)) {
		connect_thread_info.html_content = preg_replace(["'"], ['%27'], encodeURIComponent(preg_replace(['��������� .*? �� .*? �༭','&nbsp;','<em onclick="copycode\\(\\$\\(\'code0\'\\)\\);">���ƴ���</em>'], ['',' ', ''], $('postmessage_' + connect_thread_info.post_id).innerHTML)));
	}
...
function show_threadpage(pid, current, maxpage, ispreview) {
	...
	if(current < maxpage) {
		s += '<a href="javascript:;" onclick="' + clickvalue(current + 1) + '" class="nxt">��һҳ</a>';
	}
	s += '<a href="javascript:;" onclick="' + clickvalue('all') + '">�鿴����</a>';
	s += '</div></div>';
...
function show_threadindex(pid, ispreview) {
	if(!show_threadindex_data) {
		var s = '<div class="tindex"><h3>Ŀ¼</h3><ul>';
...
function showmobilebbs(obj) {
	var content = '<h3 class="flb" style="cursor:move;"><em>����������̳</em><span><a href="javascript:;" class="flbc" onclick="hideWindow(\'mobilebbs\')" title="{lang close}">{lang close}</a></span></h3><div class="c"><h4>Andriod�汾��ɨ���ά�����ֱ�����ص��ֻ�</h4><p class="mtm mbm vm"><span class="code_bg"><img src="'+ STATICURL +'image/common/zslt_andriod.png" alt="" /></span><img src="'+ STATICURL +'image/common/andriod.png" alt="������װ�а�׿ϵͳ������/HTC/С�׵��ֻ�" /></p><h4>iPhone�汾��ɨ���ά�����ֱ�����ص��ֻ�</h4><p class="mtm mbm vm"><span class="code_bg"><img src="'+ STATICURL +'image/common/zslt_ios.png" alt="" /></span><img src="'+ STATICURL +'image/common/ios.png" alt="������ƻ���ֻ�" /></p></div>';
