...
var lastusername = '', lastpassword = '', lastemail = '', lastinvitecode = '', stmp = new Array(), modifypwd = false, profileTips = '�粻��Ҫ������룬�˴�������';
...
function checkPwdComplexity(firstObj, secondObj, modify) {
	...
	firstObj.onblur = function () {
		if(firstObj.value == '') {
			var pwmsg = !modifypwd ? '����д����' : profileTips;
			if(pwlength > 0) {
	...
	firstObj.onkeyup = function () {
		if(pwlength == 0 || $(firstObj.id).value.length >= pwlength) {
			...
			errormessage(firstObj.id, '<span class="passlevel passlevel'+passlevel+'">����ǿ��:'+passlevels[passlevel]+'</span>');
		}
	};
	secondObj.onblur = function () {
		if(secondObj.value == '') {
			errormessage(secondObj.id, !modifypwd ? '���ٴ���������' : profileTips);
		}
		checkpassword(firstObj.id, secondObj.id);
	};
}
...
function checkusername(id) {
	...
	if(unlen < 3 || unlen > 15) {
		errormessage(id, unlen < 3 ? '�û����С�� 3 ���ַ�' : '�û���ó��� 15 ���ַ�');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
