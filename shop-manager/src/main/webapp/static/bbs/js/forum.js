...
function loadData(quiet, formobj) {
	...
	if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
		if(!quiet) {
			showDialog('û�п��Իָ�����ݣ�', 'info');
		}
		return;
	}

	if(!quiet && !confirm('�˲��������ǵ�ǰ�������ݣ�ȷ��Ҫ�ָ������')) {
		return;
	}
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">���»ظ������⣬����鿴', 'colspan': colspan }}};
