...
import com.rt.shop.domain.virtual.CglibBean;
...
import com.rt.shop.service.ITransportService;
 ...
 public class TransportTools
 {
   ...
   {
     Transport trans = this.transportService.selectById(
   ...
   public List<SysMap> query_cart_trans(StoreCart sc, String area_id) {
     ...
     if ((area_id != null) && (!area_id.equals(""))) {
       Area area1 = this.areaService
         .selectById(CommUtil.null2Long(area_id));
       Area area = this.areaService.selectById(area1.getParent_id());
       ...
       for (GoodsCart gc : gcList1) {
         ...
         if (goods.getGoods_transfee() == 0) {
           ...
           {
             mail_fee = mail_fee + 
               cal_order_trans(transport
               .getTrans_mail_info(), transport.getTrans_type(), goods
             ...
             ems_fee = ems_fee + 
               cal_order_trans(transport
               .getTrans_ems_info(), transport.getTrans_type(), goods
               .getGoods_weight(), goods
               .getGoods_volume(), city_name);
           }
           else
