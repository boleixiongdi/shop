     ...
     if ((json != null) && (!json.equals(""))) {
       ...
       for (Map map : list) {
         ...
         if (the_gc != null) {
           ...
           List<GoodsClass> gcChilds=goodsClassService.selectList(gc);
           for (int i = 1; i <= count; i++) {
             GoodsClass child = this.goodsClassService
               .selectById(CommUtil.null2Long(map.get("gc_id" +i)));
              gcChilds.add(child);
   ...
   public String generic_adv(String web_url, String json) {
     ...
     if ((json != null) && (!json.equals(""))) {
       ...
       if (CommUtil.null2String(map.get("adv_id")).equals("")) {
         ...
         if (advs.size() > 0) {
           if (obj.getAp_type().equals("img")) {
             if (obj.getAp_show_type() == 0) {
            	 obj.setAp_acc_id(((Advert)advs.get(0)).getAd_acc_id());
            //   obj.setAp_acc(((Advert)advs.get(0)).getAd_acc());
             ...
             if (obj.getAp_show_type() == 1) {
               ...
               obj.setAp_acc_id(((Advert)advs.get(i)).getAd_acc_id());
         ...
         Accessory sAccessory=accessoryService.selectById(obj.getAp_acc_id());
         template = template + "<a href='" + obj.getAp_acc_url() + 
           "' target='_blank'><img src='" + web_url + "/" + 
           sAccessory.getPath() + "/" + 
           sAccessory.getName() + "' /></a>";
       }
     }
